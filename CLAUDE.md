...
- **Детерминированность.** Размер = `hash(userId + Salt + датаВTimeZone)`. Один и тот же
  пользователь в течение суток всегда получает одни и те же сантиметры (и разряд) — это
  и есть «ежедневный кулдаун», отдельного состояния для него не нужно.
- **Прозвище — на каждый запрос.** В его хэш дополнительно подмешивается id inline-запроса,
  поэтому прозвище (в пределах суточного разряда) меняется при каждом вызове — так просили
  пользователи 6 августа 2026, у прошлого бота было так же. Это не `Random`: результат
  по-прежнему детерминированная функция входа, просто вход включает id запроса.
